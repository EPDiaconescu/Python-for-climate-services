...
fld='FD_gmfd_reanalysis_mean.nc'
# we put here the name of the figure to save in output folder
# here I choose to use the same name as the initial file and to replace the last 3 characters of the name with .png
imgout=fld[:-3]+'.png'

# we apply the function with the standard options
Arctic_figure(input, fld, varName, fld, output,imgout)
...
input= 'w:/30. CLIMATE SERVICES DATA PRODUCTS OFFICE/03 - Data, Code & Models/01 - Data/eccc_data/CMIP5_processed/sea_ice_concentration/'
output = 'w:/30. CLIMATE SERVICES DATA PRODUCTS OFFICE/03 - Data, Code & Models/02 - Code/python/trainingOutput/'
varName='sic'
# first we go in the directory indicated in input
os.chdir(input)
# we construct now a list containing all the file in this directory that are ended with pctl50.nc and stared  with sic_OImon_ens_rcp85
list=glob.glob('sic_OImon_ens_rcp85*pctl50.nc')

# we construct a loop, which will call the function Arctic_figure for each files we put in the list
# and we will use customized levels for the color bar
...
for fld in list[:]:
    ...
    Arctic_figure(input, fld, varName, fld, output,imgout, clevs=clevs)
