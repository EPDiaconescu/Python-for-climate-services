...
def sel_Period_Region(file,first_date, last_date, lat_bottom, lat_top, lon_left, lon_right, show_regInfo='YES', save_nerCDF='NO'):
    ...
    if show_regInfo=='YES':
        ...
        print('The first time stemp is : ' + str(dataSel.time.values[0]))
        print('The second time stemp is : ' + str(dataSel.time.values[1]))
        print('The last time stemp is : ' + str(dataSel.time.values[-1]))
        print('  ')
...
input= 'P:/30. CLIMATE SERVICES DATA PRODUCTS OFFICE/03 - Data, Code & Models/02 - Code/python/trainingData/'

# Put in output the path to the folder where you want to save the new file
output='P:/30. CLIMATE SERVICES DATA PRODUCTS OFFICE/03 - Data, Code & Models/02 - Code/python/trainingOutput/'
...
for fld in list[:]:
    print(fld)
    new_file = fld[:-3] + '_2071-2100.nc'
    sel_Period_Region(input+fld, first_date, last_date, lat_bottom, lat_top, lon_left, lon_right,
                              show_regInfo='NO', save_nerCDF='NO')
