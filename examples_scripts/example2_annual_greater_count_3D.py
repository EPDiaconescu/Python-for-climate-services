...
#####################################
def annual_greater_count_3D(file, threshold, varName, show_newPer='YES', save_nerCDF='NO'):
    """ This function will open an 3D netcDF file, for each year and for each grid point counts the number of values
    greater than the threshold and save the new file in netCDF.
...
for fld in list[:]:
    ...
    annual_greater_count_3D(input+fld, threshold, varName,show_newPer='NO', save_nerCDF=output+fileout)
